  ...
  pid_incr_init(&pid_standard,25,0.15,0,15000,500);  //3508速度环
	...
	while(1)
	{
		...
		free_move();
		 ...
		 {
			 if ( rc->L_y == -660 )
				 moto_can.standard = pid_incr_calc(&pid_standard,( Encoder_standard->ecd_rate/19.f),rc->L_x*2);
			 else
		   moto_can.standard = 0;
		 }
     else
...
void standart_mode_rand(void)
{
	...
	else flag_cm1.rand_time_flag ++;

	//边缘碰撞
	if( (encoder_cm->all_echo > encoder_cm->max_echo - 800 && flag_cm1.rand_speed > 0) || 
		 ( encoder_cm->all_echo <  800 && flag_cm1.rand_speed <0 ))
		  flag_cm1.rand_speed = - flag_cm1.rand_speed;
	
	
	    //对输出滤波
    	flag_cm1.rand_speed = ( 1 - kp_pass ) * flag_cm1.rand_last_speed + kp_pass * flag_cm1.rand_speed;  

			//moto_can.standard = pid_incr_calc( &pid_standard,( Encoder_standard->ecd_rate/19.f),flag_cm1.rand_speed ); 
	    //flag_cm1.rand_last_speed = flag_cm1.rand_speed;
}
