   ...
   pid_init(&pid_yaw_cirle,20,0,0,200,50,50,200);  //6020角度环
	 ...
	 // pid_init(&pid_6020_pitch,235,5,0,30000,5000,10000,30000);  //6020速度环
	  //pid_init(&pid_pitch_cirle,20,0,0,200,50,100,200);  //6020角度环
		...
		pid_init(&pid_6020_pitch,200,1.5,200,27000,2500,10000,30000);  //6020速度环
	  pid_init(&pid_pitch_cirle,19,0.3,20,200,30,80,250); 
		
  //pid_init(&pid_6020_pitch,220,5,0,30000,5000,10000,30000);  //6020速度环
	//pid_init(&pid_pitch_cirle,20,0.3,0,220,34,100,250);  //6020角度环
	  //得到各处指针地址，信息获取
