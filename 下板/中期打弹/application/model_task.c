...
{ 
  ...
  pid_init(&pid_yaw_cirle,20,0,0,200,50,50,200);  //6020角度环
	 ...
	 //pid_init(&pid_6020_pitch,190,0.5,190,23000,7000,5000,30000);  //6020速度环
	  pid_init(&pid_6020_pitch,210,0.3,130,25000,5000,7000,30000); 
	  //pid_init(&pid_pitch_cirle,20,0.1,20,200,30,100,300); 
		 pid_init(&pid_pitch_cirle,25,0.2,30,250,30,80,250); 
	  //得到各处指针地址，信息获取
...
{	
	...
	setz  = setz  < -20 ? -20 : setz ;
	setz  = setz  >  55 ?  55 : setz ;  //云台pitch限位	
	
	 /////////////////               yaw          /////////////

	 moto_can->yaw_cirle =	Calculate_Current_Value( &pid_yaw_cirle, setx , bmi->yaw );
	 moto_can->yaw  =	pid_incr_calc( &pid_6020_yaw,task.speed_x,moto_can->yaw_cirle );
		
	 ////////////////                pitch       ///////////    
	 moto_can->pitch_cirle = Calculate_Current_Value( &pid_pitch_cirle, setz , bmi->pit ); //角度环  
	 //moto_can->pitch  =	-Calculate_Current_Value( &pid_6020_pitch, setz, task.speed_y );
...
void vis_cruise(void)
{
	if( chassic->bee_flag == 0)
	   TIM_SetCompare3(TIM4,0);
		 task.vis_time++;
					 if( task.vis_time >= 1200 )
					 {
						 //yaw轴巡航
					   moto_can->yaw  =	pid_incr_calc( &pid_6020_yaw,task.speed_x, -80 );
						 //moto_can->yaw  =	pid_incr_calc( &pid_6020_yaw,task.speed_x, 0 );
						//pitch轴巡航						
									 if( task.flag_pit == 0){
										if( bmi->pit  < 13 )
										{
