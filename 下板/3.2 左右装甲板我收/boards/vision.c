...
#define RX_VISION_SIZE 32		//DMA接收缓冲区大小

 static u8 RXVISION[RX_VISION_SIZE];		//DMA接收缓冲区
 u8 rx_index = 0;
 u8 rx_data_buf[128];
  vision vis;
 static motor_date_ty * stand_moto;
 static mpu*bmi;
...
void vision_init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	stand_moto = getstandmotorpoint();   //获得底盘电机信息
...
float look_set_right = 0;
float look_set_left = 0;
...
void DMA2_Stream1_IRQHandler(void)
{
	...
	if(DMA_GetFlagStatus(DMA2_Stream1,DMA_FLAG_TCIF1)!=RESET)
	{
		...
		while((rx_data_buf[offset]!=0xA5) && (rx_index-offset>=RX_VISION_SIZE)  ){offset++;}
		 ...
		 if( rx_data_buf[offset] == 0xA5 && (rx_data_buf[offset+15] == 0xEE)  )
	 {
		
		 vis.mode = rx_data_buf[1+offset];     //模式
		 ///////////yaw轴///////////
		 vis.set_yaw_data.data[0] = rx_data_buf[2+offset] ;  
		 vis.set_yaw_data.data[1] = rx_data_buf[3+offset] ;  
     vis.set_yaw_data.data[2] = rx_data_buf[4+offset] ;  
		 vis.set_yaw_data.data[3] = rx_data_buf[5+offset] ; 
		 if( vis.set_yaw_data.angle == 45 )
			 vis.set_yaw_data.angle = 0;
		 vis.set_yaw_data.angle =  -vis.set_yaw_data.angle;
		 
		 look_set_yaw = vis.set_yaw_data.angle;
  	 ...
  	 ////////////yaw_right轴///////////
		 vis.set_right_yaw.data[0]  =  rx_data_buf[10+offset] ;  
		 vis.set_right_yaw.data[1]  =  rx_data_buf[11+offset] ;
		 vis.set_right_yaw.data[2]  =  rx_data_buf[12+offset] ;
		 vis.set_right_yaw.data[3]  =  rx_data_buf[13+offset] ;
		 
		 if( vis.set_right_yaw.angle == 45 )
			 vis.set_right_yaw.angle = 0; 
			look_set_right = 	 vis.set_right_yaw.angle;
		 
		 vis.command = rx_data_buf[14+offset];   //识别标志位
		 
  	 ////////////yaw_left轴///////////
		 vis.set_left_yaw.data[0]  =  rx_data_buf[16+offset] ;  
		 vis.set_left_yaw.data[1]  =  rx_data_buf[17+offset] ;
		 vis.set_left_yaw.data[2]  =  rx_data_buf[18+offset] ;
		 vis.set_left_yaw.data[3]  =  rx_data_buf[19+offset] ;
		 
		 
		 if( vis.set_left_yaw.angle == 45 )
			 vis.set_left_yaw.angle = 0; 
			look_set_left = 	 vis.set_left_yaw.angle;
				 
				 
		 if( stand_moto->ecd_rate >= 0 )   //往右跑（正），往左瞄（负）
		    vis.set_yaw = vis.set_yaw_data.angle + 0.1f*( 1.0f/( vis.set_left_yaw.angle - vis.set_right_yaw.angle)) ;
		 else                              //往左跑（负），往右瞄（正）
        vis.set_yaw = vis.set_yaw_data.angle + 0.1f*( 1.0f/( vis.set_left_yaw.angle - vis.set_right_yaw.angle));
